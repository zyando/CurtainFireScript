...
facelist = [[normalize(obj.veclist[i]) for i in face] for face in obj.face_index_list]

ico0_veclist = objvertices("ico.obj", 0)
ico4_veclist = objvertices("ico.obj", 4)

def task():
	vec_to_target = REIMU_CENTER_BONE.WorldPos - CENTER_BONE.WorldPos
	
	for vec in ico4_veclist:
		if dot(vec, normalize(vec_to_target)) > 0.7: continue
	...
	parent.SetMotionInterpolationCurve(Vector2(0.3, 0.7), Vector2(0.3, 0.7), 30)
