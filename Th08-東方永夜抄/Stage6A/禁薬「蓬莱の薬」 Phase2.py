# -*- coding: utf-8 -*-

veclists = [objvertices("ico.obj", i) for i in range(5)]

phase_length = 1020

binder = [Quaternion.Identity]
def rotate(rot = Quaternion.RotationAxis(Vector3.UnitY, RAD * (270.0 / phase_length))): binder[0] *= rot
WORLD.AddTask(rotate, 0, 0, 0)

def shot_rice():
	for vec in veclists[2]:
		shot = EntityShot(WORLD, "RICE_M", 0xA00000)
		shot.Pos = CENTER_BONE.WorldPos
		shot.Velocity = vec * binder[0] * 12
		shot.LivingLimit = 100
		shot()
WORLD.AddTask(shot_rice, 5, 0, 0)

def shot_dia():
	for vec in veclists[4]:
		shot = EntityShot(WORLD, "DIA_BRIGHT", 0x400000)
		shot.Pos = CENTER_BONE.WorldPos
		shot.Velocity = vec * binder[0] * 12
		shot.LivingLimit = 100
		shot()
WORLD.AddTask(shot_dia, 40, 0, 0)

def shot_dia_to_target(veclist = [v for v in veclists[1] if v.z < -0.8]):
	mat = Matrix3.LookAt(REIMU_CENTER_BONE.WorldPos - CENTER_BONE.WorldPos, Vector3.UnitY)

	for vec in veclist:
		for i in range(3):
			shot = EntityShot(WORLD, "DIA_BRIGHT", 0x000040)
			shot.Pos = CENTER_BONE.WorldPos
			shot.Velocity = vec * mat * (1 + i * 0.5) * 2
			shot.LivingLimit = 400
			shot()
WORLD.AddTask(shot_dia_to_target, 5, 0, 622)
