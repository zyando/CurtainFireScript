...
def shot_dia_group_task():
		...
		for i in range(way):
			...
			def replace(orgn_shot = shot, mat = Matrix3.RotationAxis(axis, RAD * (i % 2 * 2 - 1) * 30)):
				...
				def move():
					...
					shot.Velocity = shot.LookAtVec * mat * 4
				return move
	...
	replace_func_list1 = [shot_dia_group(vec, axis, 16, 100, 12, 0x000040, 0xA00000) for vec in veclist for axis in axislist[::-1] if abs(dot(vec, axis)) < 0.95]
	replace_func_list2 = [shot_dia_group(vec, axis, 16, 150, 16, 0x400000, 0x0000A0) for vec in veclist[::-1] for axis in axislist if abs(dot(vec, axis)) < 0.95]
