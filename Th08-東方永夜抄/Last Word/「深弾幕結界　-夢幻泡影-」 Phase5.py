...
def task():
	for vec, axis in vec_axis_list:
		for i in [1, 1.1]:
			...
			get_speed1 = lambda t: 6.0 + t * i * 1,
			get_speed2 = lambda t: 12.0,
			
			get_vec_rot = lambda t, axis = axis, begin = 0.0, end = -20: Quaternion.RotationAxis(axis, RAD * (begin + (end - begin) * t)),
			init_pos = RAD * 0, interval_pos = RAD * 3,
			...
			get_restart_frame = lambda t: 320 * (1 - t * 0.6),
			)
