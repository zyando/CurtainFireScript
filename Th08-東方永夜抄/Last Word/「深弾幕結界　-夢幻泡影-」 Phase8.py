...
def task():
	for vec, axis in vec_axis_list:
		for i in [1, 1.1]:
			...
			get_speed1 = lambda t, i = i: 14.0 + t * i * 6,
			get_speed2 = lambda t: 16.0,
			
			get_vec_rot = lambda t, axis = axis, begin = -1, end = -8: Matrix3.RotationAxis(axis, RAD * (begin + (end - begin) * t ** 2)),
			init_pos = RAD * 0, interval_pos = RAD * 4,
			
			num_shot = 180, interval_shot = 1,
			num_task = 1, interval_task = 180,
			
			#停止するフレームを返す関数、引数は最初の弾を0、最後の弾を1とした場合の浮動小数点
			#実際の停止するフレームは、中心に到達するまでに経過するフレーム * pause_frame_func(t)で求められる
			get_pause_frame = lambda t: 0.4 + t * 1,
			#再発進するフレームを返す関数、引数は最初の弾を0、最後の弾を1とした場合の浮動小数点
			get_restart_frame = lambda t: 270 * (1 - t * 0.42),
			)
