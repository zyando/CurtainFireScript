...
def shot_red_amullet(mat = [Matrix3.Identity, Matrix3.RotationAxis(Vector3.UnitZ, RAD * 4)]):
    ...
    for vec in veclist:
        ...
        shot.Velocity = vec * HAND_BONE.WorldMat * 12.0
...
def shot_homing_amulet(speed = 12):
    ...
    for vec in homing_veclist:
        shot = EntityShot(WORLD, "AMULET", 0xA000A0)
        shot.Pos = Vector4(0, 0, 4, 1) * HAND_BONE.WorldMat
        ...
        def homing(shot = shot):
            ...
            shot.LivingLimit = shot.FrameCount + int((TARGET_BONE.WorldPos - shot.Pos).Length() / speed)
