...
def spawn_particle(bone, color, size, interval = 0.5):
	...
	for i in range(num):
		t = (1.0 / num) * i
		particle = EntityShot(WORLD, "PARTICLE", color, size)
	...
	pos_binder[bone.EntityId] = pos
WORLD.AddTask(lambda: [spawn_particle(bone, 0x404040, 0.5) for bone in OPTION_BALL_BONES], 0, 0, 0)